...
proj_field = ds.derived_field_list[field_it]
    ...
    if make_pickle == True:
        ...
        for proj_it in yt.parallel_objects(range(len(projection_vectors)), njobs=int(len(vectors_along_cone))):# range(len(projection_vectors)):
            if True in np.isnan(projection_vectors[proj_it]):
                ...
                proj_rank_int = int(rank/(8*len(field_list)))
                proj_rank_rem = np.remainder(rank,(8*len(field_list)))
                field_rank_int = int(proj_rank_rem/len(field_list))
                proj_root_rank = proj_rank_int*(8*len(field_list)) + field_rank_int*len(field_list)

                #Fields to project
                proj_dict = {field_list[0][1]:[], field_list[1][1]:[], field_list[2][1]:[], field_list[3][1]:[]}
                ...
                if rank == proj_root_rank and size > 1:
                    for kit in range(1,len(proj_dict_keys)):
                        ...
                        print("Updated proj_dict on rank", proj_root_rank, "from pickle", pickle_file + 'proj_data_' +str(proj_root_rank) +str(kit)+'.pkl')
                        os.remove(pickle_file + 'proj_data_' +str(proj_root_rank) +str(kit)+'.pkl')
                    '''
                    updated_proj_dict_key = np.zeros(len(field_list), dtype=bool)
                    updated_proj_dict_key[0] = True
                    #check to see if all proj files exist yet
                    check_counter = 0
                    while False in updated_proj_dict_key:
                        for kit in range(1,len(proj_dict_keys)):
                            try:
                                if updated_proj_dict_key[kit] == False:
                                    file = open(pickle_file + 'proj_data_' +str(proj_root_rank) +str(kit)+'.pkl', 'rb')
                                    key, proj_array = pickle.load(file)
                                    file.close()
                                    proj_dict[key] = proj_array
                                    updated_proj_dict_key[kit] = True
                                    print("Updated proj_dict on rank", proj_root_rank, "from pickle", pickle_file + 'proj_data_' +str(proj_root_rank) +str(kit)+'.pkl')
                                    os.remove(pickle_file + 'proj_data_' +str(proj_root_rank) +str(kit)+'.pkl')
                            except:
                                pass
